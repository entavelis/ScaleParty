...
@METRICS.register_module()
class MultiscaleMetrics(FID):
    def __init__(self, num_images, image_shape=None, inception_pkl=None, bgr2rgb=True, inception_args=dict(normalize_input=False)):
        super().__init__(num_images, image_shape=image_shape, inception_pkl=inception_pkl, bgr2rgb=bgr2rgb, inception_args=inception_args)
        self.fake_feats_2 = []
        self.ssims = []
    ...
    @staticmethod
    def calculate_ssim(batch):

        halves = []
        calc_size = batch[0].shape
        for i, minibatch in enumerate(batch):
            if i: #for the second
                minibatch = F.interpolate(minibatch, size=(calc_size[-2], calc_size[-1]))

            minibatch = ((minibatch + 1) / 2)
            minibatch = minibatch.clamp_(0, 1)
            half1 = minibatch.cpu().data.numpy().transpose((0, 2, 3, 1))
            half1 = (half1 * 255).astype('uint8')
            halves.append(half1)
        score = ms_ssim(halves[0], halves[1])

        return score 


    @torch.no_grad()
    def feed_op(self, batch, mode, end):
        """Feed data to the metric.
        ...
        ssim = self.calculate_ssim(batch)

        self.ssims.append(ssim)

        for b in batch:
            feat = self.feed_op_one(b[:end]) 
            feats.append(feat)
        ...
        if (dist.is_initialized()
            ...
            if mode == 'reals':
                ...
                self.fake_feats.append(feats[0].cpu())
                self.fake_feats_2.append(feats[1].cpu()) 
            else:
                raise ValueError(
                    f"The expected mode should be set to 'reals' or 'fakes,\
                    but got '{mode}'")
    ...
    def summary(self):
        ...
        # calculate fake inception stat for larger
        fake_feats = torch.cat(self.fake_feats_2, dim=0)
        assert fake_feats.shape[0] >= self.num_images
        fake_feats = fake_feats[:self.num_images]
        fake_feats_np = fake_feats.numpy()
        fake_mean = np.mean(fake_feats_np, 0)
        ...
        # calculate distance between real and fake statistics for larger
        fid_2, mean_2, cov_2 = self._calc_fid(fake_mean, fake_cov, self.real_mean,
                                        self.real_cov)

        # calculate average of ssim scores
        ssim = np.mean(self.ssims)

        # calculate inception distance between image pairs
        inc_dist = np.mean([F.l1_loss(f1,f2).numpy() for f1,f2 in zip(self.fake_feats, self.fake_feats_2)])
        # inc_dist = 0.


        # results for print/table
        self._result_str = (f'{fid:.4f} ({mean:.5f}/{cov:.5f}, {fid_2:.4f} ({mean_2:.5f}/{cov_2:.5f}), ssim:{ssim}')
        # results for log_buffer
        self._result_dict = dict(fid=fid, fid_mean=mean, fid_cov=cov, fid_x2=fid_2, fid_x2_mean=mean_2, fid_x2_cov=cov_2, ssim=ssim, inc_dist = inc_dist)
