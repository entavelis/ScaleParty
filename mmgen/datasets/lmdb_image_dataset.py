...
class LMDBImageDataset(Dataset):
    ...
    def load_annotations(self):
        ...
        if not self.env:
            raise IOError('Cannot open lmdb dataset', self.imgs_root)
    ...
    def prepare_data(self, idx):
        with self.env.begin(write=False) as txn:
            # key = f'{self.full_resolution}-{str(index).zfill(5)}'.encode('utf-8')
            key = f'{1024}-{str(idx).zfill(5)}'.encode('utf-8')
            img_bytes = txn.get(key)


        results = dict(real_img_path=key)
